...
// Called after every candidate answer. Sends the full conversation so
// far to Claude Sonnet, gets a reaction + the next question back.
//
// Ending logic: instead of fragile string-matching on phrases like
// "thank you" or "that's all", the model is told to plan for roughly
// TOTAL_QUESTIONS questions, and when it decides to wrap up, to append
// an exact invisible marker at the very end of its message. We detect
// that marker server-side, strip it before it's ever shown or spoken,
// and return a clean `isFinal` boolean the frontend can trust.
...
const TOTAL_QUESTIONS = 6; // including the opening question
...
- Do not use markdown or lists.`;
    ...
    const isFinal = raw.includes(END_MARKER);
